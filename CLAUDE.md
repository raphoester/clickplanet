...
The stack includes a **postgres**: the backend keeps the whole tile map in process and writes what changed to postgres every second. The ledger goes there too, the antibot keeps its bans and evidence there in its own schema, and the chat writes each message there before broadcasting it. The `tile_state` volume only holds the pre-postgres files the first boot imports. See [`apps/backend/CLAUDE.md`](apps/backend/CLAUDE.md) for the durability tradeoff and the full config schema.
