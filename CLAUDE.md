...
The stack includes a **postgres**: the backend keeps the whole tile map in process and writes what changed to postgres every second. The ledger goes there too, and the antibot keeps its bans and evidence there in its own schema. The chat log is still a file on the `tile_state` volume, until it moves too. See [`apps/backend/CLAUDE.md`](apps/backend/CLAUDE.md) for the durability tradeoff and the full config schema.
