...
The stack includes a **postgres**: the backend keeps the whole tile map in process and writes what changed to postgres every second, and writes each chat message there before broadcasting it. The ledger, bans and antibot evidence are still files on the `tile_state` volume, until they move too. See [`apps/backend/CLAUDE.md`](apps/backend/CLAUDE.md) for the durability tradeoff and the full config schema.
