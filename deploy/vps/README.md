...
included, and Caddy already forwards it. `SendMessage` needs a
click token that names an account, so a sender first passes Turnstile, and the
server picks the name: a username, or `guest_` and the account's code. The
things that keep it usable are all config:
