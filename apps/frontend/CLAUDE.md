...
npm run og-image     # Redraw the generic social card from static/og-source.png
npm run og-countries # Redraw one social card per country (see "Link previews")
```
...
Docker (only for the local full stack in `deploy/`; production is a Cloudflare
...
fallback that `wrangler.jsonc` sets with `not_found_handling`. The one thing it
does *not* mirror is the Worker: nginx serves `index.html` as it is, so a
`?c=<country>` link gets the generic card there. See [Link
...
- `shareCard.ts` — what a link with a country on it previews as, and the only
  place a `?c=` value is turned into a country. Read by `worker/index.ts` at the
  edge and by the share button in the app; see [Link previews](#link-previews).
...
the buttons against the other. `/static/og/<code>.jpg` is the same screenshot
with one country's flag laid over it, one per country; see [Link
previews](#link-previews). Both sizes come from `CARD_WIDTH` / `CARD_HEIGHT` in
`shareCard.ts`, so there is one number to change and a test that holds
`index.html` to it.
...
A link with a country on it — `https://clickplanet.lol/?c=de` — previews as
*Germany*: German flag, German title, its own `og:url`. That is what makes the
game spread, because the link a player pastes into a group of their compatriots
shows their own flag rather than a card about a game they have never heard of.

**Scrapers do not run JavaScript**, so this cannot be done from React. The tags
have to be right in the bytes the server returns, which means Cloudflare:
`worker/index.ts` runs in front of the static assets and rewrites the `content`
of the og/twitter tags already in `index.html`, with `HTMLRewriter` so the
document streams rather than being held in memory.

`shareCard.ts` is the part worth reading. It holds the parameter name, the
allowlist, the card's copy, and the list of which tag each field goes in — the
Worker only walks that list. `SHARE_COUNTRY_PARAM` and `shareUrl` are there for
the in-app share button to write its links with, so the parameter it emits and
the codes the Worker accepts cannot drift apart.

- **Nothing from the query string reaches a meta tag.** `?c=` is normalised and
  looked up in the country list; the card is built from the `Country` that comes
  back. An unknown code, a malformed one, or none at all returns *no card*, and
  the Worker hands back the asset untouched — so the generic card lives in
  `index.html` alone rather than in a second copy in the Worker.
- **The cards are drawn at build time, not per request** — `npm run og-countries`
  writes 255 of them, ~25 kB each, and they are committed like `og-image.jpg` is.
  sharp does not run in a Worker, so the alternative was a Workers-compatible
  rasteriser and a per-request image. **The cost of that choice is that no card
  carries live standings** — not the rank, not the tile count. They cannot be
  baked in either: there is no leaderboard endpoint to read at build time, only
  the whole ownership map, which the client ranks itself. If the numbers are
  wanted later, the place to put them is `og:title` / `og:description`, which
  the Worker writes per request and could fill from a cached fetch. A card with
  no rank on it still beats one generic card for every country.
- **Every card is 1200×627**, the same as the generic one, which is why the
  Worker rewrites the image URL and leaves `og:image:width` / `og:image:height`
  alone. Draw them at another size and those two tags join the list.
- **The rewritten document goes out with no ETag**, and the Worker asks the
  asset handler unconditionally. Every copy of `index.html` carries the one
  ETag, so without this a scraper revalidating `/?c=fr` gets a 304 and keeps
  whichever card it already had.
- **Only documents reach the Worker.** `run_worker_first` in `wrangler.jsonc`
  excludes `/assets/*` and `/static/*`, so the bundles, the textures and the
  4.9 MB coordinates blob are still served straight from the asset store.

`npx wrangler dev` is how to see it; `curl -s 'http://127.0.0.1:8787/?c=de' |
grep og:` is the whole check, plus confirming the image URL it names actually
answers 200. A card that validates but 404s on its image is the usual failure.
