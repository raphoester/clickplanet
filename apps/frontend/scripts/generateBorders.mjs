...
//
//   npm run borders [-- path-to-geojson]
//
...
// hand, not on every build. It writes static/borders-<hash>.bin and the module
// naming it, the same content-addressed pair as the coordinates blob and the
// atlas — `public/_headers` caches /static/* for a week, so a regenerated file
// under a stable name would be served stale.
...
// --- tiles. Found rather than named, so this stays in step with whatever the
// coordinates generator last wrote.
const coordinates = fs.readdirSync("static").filter((f) => /^coordinates-[0-9a-f]{8}\.bin$/.test(f))
if (coordinates.length !== 1) {
    throw new Error(`expected one static/coordinates-<hash>.bin, found ${coordinates.length}`)
}
const buf = fs.readFileSync(`static/${coordinates[0]}`)
...
const fileName = `borders-${hash}.bin`
// The backend reads it too, for the admin tools, so /map holds the source like the coordinates blob.
for (const dir of ["static", "../../map"]) {
    for (const entry of fs.readdirSync(dir)) {
        if (/^borders-[0-9a-f]{8}\.bin$/.test(entry) && entry !== fileName) fs.unlinkSync(`${dir}/${entry}`)
    }
    fs.writeFileSync(`${dir}/${fileName}`, bytes)
}
fs.writeFileSync("src/app/viewer/bordersAsset.ts",
    `// Generated by scripts/generateBorders.mjs — do not edit by hand.
// Regenerate with \`npm run borders\`.

export const BORDERS_URL = "/static/${fileName}"
`)
console.error(`wrote static/${fileName} and ../../map/${fileName}: ${codes.length - 1} landmasses, ${bytes.byteLength} bytes`)
console.error("run the backend's `make map` and commit all three copies")
