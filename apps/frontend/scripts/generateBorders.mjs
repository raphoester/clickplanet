...
for (const entry of fs.readdirSync("static")) {
    if (/^borders-[0-9a-f]{8}\.bin$/.test(entry) && entry !== fileName) fs.unlinkSync(`static/${entry}`)
}
...
`)
console.error(`wrote static/${fileName}: ${codes.length - 1} landmasses, ${bytes.byteLength} bytes`)
