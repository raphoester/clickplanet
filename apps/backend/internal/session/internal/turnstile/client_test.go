...
func TestAGoodTokenIsAccepted(t *testing.T) {
	...
	assert.NoError(t, client.Verify(t.Context(), "a-token", "203.0.113.7"))
}
...
func TestTheSecretAndTheTokenAreSentAsAForm(t *testing.T) {
	...
	require.NoError(t, client.Verify(t.Context(), "a-token", "203.0.113.7"))
...
func TestATokenForAnotherActionOrHostnameIsRefused(t *testing.T) {
	...
	assert.ErrorIs(t, otherHost.Verify(t.Context(), "a-token", ""), ErrRefused)
}
...
func TestAnEmptyOrOversizedTokenIsRefusedWithoutCallingSiteverify(t *testing.T) {
	...
	assert.ErrorIs(t, client.Verify(t.Context(), "", ""), ErrRefused)
	assert.ErrorIs(t, client.Verify(t.Context(), strings.Repeat("a", maxTokenLength+1), ""), ErrRefused)
...
func TestTheSecretIsNotInTheRefusal(t *testing.T) {
	...
	err := client.Verify(t.Context(), "a-token", "")
	require.Error(t, err)
