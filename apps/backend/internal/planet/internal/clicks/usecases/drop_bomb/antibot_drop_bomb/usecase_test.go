...
type stubUseCase struct{ in drop_bomb.In }
...
func TestABannedCallersBombIsADud(t *testing.T) {
	...
	_, err := decorator.Execute(cpctx.AddIPToContext(t.Context(), "2001:db8::9"), drop_bomb.In{CountryID: "fr"})
...
func TestAnyoneElsesBombIsReal(t *testing.T) {
	...
	_, err := decorator.Execute(cpctx.AddIPToContext(t.Context(), "203.0.113.7"), drop_bomb.In{CountryID: "fr"})
	require.NoError(t, err)
