...
func (s stubUseCase) Execute(context.Context, reassign_country.In) (reassign_country.Out, error) {
	return s.out, s.err
}
...
func TestAReassignmentIsLoggedWithItsCounts(t *testing.T) {
	want := reassign_country.Out{FromBefore: 22040, Moved: 22040, ToAfter: 22040}
...
func TestADryRunIsLoggedToo(t *testing.T) {
	...
	_, err := useCase.Execute(t.Context(), reassign_country.In{From: "dz", To: "fr", DryRun: true})
	require.NoError(t, err)
...
func TestAFailureIsLoggedWithHowFarItGot(t *testing.T) {
	...
	useCase, logs := audited(stubUseCase{out: reassign_country.Out{Moved: 512}, err: cause})
