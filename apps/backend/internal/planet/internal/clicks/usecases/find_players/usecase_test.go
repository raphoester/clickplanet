...
func TestItListsWhoseFlagStillHoldsInTheAreaLatestFirst(t *testing.T) {
	...
	useCase := find_players.New(book, current, israel, bans{"bot": {Offence: 2, Until: start.Add(time.Hour)}}, countries{})

	out, err := useCase.Execute(t.Context(), find_players.In{Flag: "ps", Area: "il"})
	require.NoError(t, err)

	assert.Equal(t, find_players.Out{Total: 2, Players: []find_players.Player{
		{
...
func TestNoAreaIsTheWholeMapAndTheLimitCuts(t *testing.T) {
	book, current := setup(t)
	useCase := find_players.New(book, current, israel, nil, countries{})

	out, err := useCase.Execute(t.Context(), find_players.In{Flag: "ps", Limit: 1})
	require.NoError(t, err)
...
func TestItRefusesAnUnknownFlagOrArea(t *testing.T) {
	...
	_, err := useCase.Execute(t.Context(), find_players.In{Flag: ""})
	require.ErrorIs(t, err, clicks.ErrUnknownCountry)

	_, err = useCase.Execute(t.Context(), find_players.In{Flag: "ps", Area: "israel"})
	require.ErrorIs(t, err, clicks.ErrUnknownCountry)
}
