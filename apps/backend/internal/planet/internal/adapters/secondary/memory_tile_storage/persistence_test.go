...
func (s *testSuite) TestRunFlushesOnShutdown() {
	persistence := newFakePersistence()
	storage := s.newStorageOn(memory_tile_storage.Config{FlushInterval: time.Hour}, persistence)
...
func (s *testSuite) TestRunFlushesOnItsInterval() {
	persistence := newFakePersistence()
	storage := s.newStorageOn(memory_tile_storage.Config{FlushInterval: 10 * time.Millisecond}, persistence)
