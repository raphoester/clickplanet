...
func (s *stubUseCase) Execute(_ context.Context, in revert_player.In) (revert_player.Out, error) {
	s.in = in
	return s.out, s.err
}
...
func TestTheRequestReachesTheUseCaseAndTheCountsComeBack(t *testing.T) {
	useCase := &stubUseCase{out: revert_player.Out{Scope: "9.9.9.9", Touched: 40, Held: 31, Restored: 30}}

	res, err := revert(t, useCase)
	require.NoError(t, err)

	assert.Equal(t, revert_player.In{Scope: "9.9.9.9", DryRun: true}, useCase.in)
