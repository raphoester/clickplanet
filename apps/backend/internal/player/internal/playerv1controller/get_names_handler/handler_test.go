...
func TestNamesAreAnsweredByAccountIdAndTheRestLeftOut(t *testing.T) {
	...
	require.NoError(t, store.SaveProfile(t.Context(), players.Profile{Account: ada, Name: "Ada", UpdatedAt: time.Now()}))

	res, err := get_names_handler.New(get_names_usecase.New(store)).GetNames(t.Context(),
