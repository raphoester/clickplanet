...
// what the map does.
func (h *harness) after(gap time.Duration) (antibot.Verdict, antibot.Evidence) {
	h.clock.now = h.clock.now.Add(gap)
	h.tile++

	return h.watchdog.Watch(antibot.Click{
		Scope:   "caller",
		Tile:    h.tile,
		Country: "FR",
		At:      h.clock.now,
...
func (h *harness) beat(gap time.Duration, count int) (antibot.Verdict, antibot.Evidence) {
	var (
...
func TestAShortSteadyRunIsOnlySuspect(t *testing.T) {
	...
	assert.Equal(t, antibot.Suspect, verdict, "half a minute of rhythm is not proof")
}
...
func TestAHandIsNotAClock(t *testing.T) {
	...
	assert.Equal(t, antibot.Clear, verdict, "the spread of a hand is the whole defence")
}

func TestLookingAwayOnceEndsTheRun(t *testing.T) {
	h := newHarness(config())

	require.Equal(t, antibot.Certain, func() antibot.Verdict { v, _ := h.beat(time.Second, 700); return v }())
	...
	assert.Equal(t, antibot.Clear, verdict, "the new run is judged on its own length")

	verdict, _ = h.after(time.Second)
	assert.Equal(t, antibot.Suspect, verdict, "and has to earn its way back up from Clear")
}
