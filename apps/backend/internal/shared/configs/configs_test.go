...
func TestTheFileFillsTheStruct(t *testing.T) {
	...
	require.NoError(t, configs.Load(&config, configs.FromFile(write(t, `
httpServer:
...
func TestTheEnvironmentAloneIsAWholeConfig(t *testing.T) {
	...
	var config testConfig
	require.NoError(t, configs.Load(&config))
...
func TestAMissingFileIsRefused(t *testing.T) {
	var config testConfig
	require.Error(t, configs.Load(&config, configs.FromFile("no-such-file.yaml")))
}
...
func TestAConfigThatRefusesItselfStopsTheLoad(t *testing.T) {
	...
	err := configs.Load(&config, configs.FromFile(write(t, "gameMap:\n  maxIndex: 0\n")))

	require.Error(t, err)
	assert.ErrorIs(t, err, configs.ErrValidation)
...
func TestAConfigWithNoValidateIsLoadedAnyway(t *testing.T) {
	var config testConfig
	require.NoError(t, configs.Load(&config, configs.FromFile(write(t, "gameMap:\n  maxIndex: 1\n"))))
}
