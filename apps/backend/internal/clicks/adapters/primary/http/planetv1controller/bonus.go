...
// BonusRegistry is the local port onto internal/clicks/domain/bonus, declared
...
type ClickBooster interface {
	Boost(key string, multiplier float64, until time.Time) ratelimit.State
}
...
func (s *ClickService) ClaimBonus(
	...
	scope := connectutil.RateLimitKey(ctx)
