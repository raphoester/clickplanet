...
func (s *stubBooster) Boost(key string, multiplier float64, until time.Time) ratelimit.State {
...
func claim(t *testing.T, client planetv1connect.ClickServiceClient, token string) (*planetv1.ClaimBonusResponse, error) {
	...
	res, err := client.ClaimBonus(context.Background(), connect.NewRequest(&planetv1.ClaimBonusRequest{
		Token:     token,
		CountryId: "fr",
	}))
	if err != nil {
...
func TestClaimingABonusStartsTheBoost(t *testing.T) {
	...
	booster := &stubBooster{state: ratelimit.State{Tokens: 4, Capacity: 30, PerSecond: 3}}
...
func TestTheAnswerCarriesTheWidenedAllowance(t *testing.T) {
	...
	bonuses := &stubBonuses{claimable: true, reward: bonus.Reward{Kind: bonus.KindTripleClicks, Duration: time.Minute}}
	booster := &stubBooster{state: ratelimit.State{Tokens: 7, Capacity: 30, PerSecond: 3}}
