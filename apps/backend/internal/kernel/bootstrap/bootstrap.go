// Package bootstrap runs a set of bounded contexts as one process.
//
...
func Run(ctx context.Context, options Options) error {
	...
	routes.mountOn(router, httpserver.MiddlewareStack(
		httpserver.NewLoggingMiddleware(options.Logger),
...
func buildModules(
	...
	for _, module := range options.Modules {
		...
		options.Logger.Debug("module built",
			lf.String("module", module.Name),
			lf.Int("runners", runners.count()-before),
		)
	}
