...
func mountMetrics(router *http.ServeMux, metrics *prometheus.Registry, logger logging.Logger) {
	metricsRouter := http.NewServeMux()
	metricsRouter.HandleFunc("GET /", prom.HandlerForRegistry(metrics).ServeHTTP)

	router.Handle("/metrics", httpserver.MiddlewareStack(
...
func serve(
	...
	options.Logger.Info("Listening", lf.String("address", server.Addr))
	...
	case sig := <-signals:
		options.Logger.Info("shutting down", lf.String("signal", sig.String()))

	case <-ctx.Done():
		options.Logger.Info("shutting down", lf.String("reason", "context cancelled"))
	}
	...
	if err := server.Shutdown(shutdownCtx); err != nil {
		options.Logger.Error("failed to shut down the http server", lf.Err(err))
	}
...
func startRunners(ctx context.Context, runners *runnerRegistry, logger logging.Logger) *sync.WaitGroup {
	...
	for _, runner := range runners.runners {
		...
		go func() {
			defer started.Done()
			runner.run(ctx)
			logger.Debug("runner stopped", lf.String("runner", runner.name))
		}()
	}
