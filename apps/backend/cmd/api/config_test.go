...
func TestTheExampleConfigStillCarriesTheRestOfTheFile(t *testing.T) {
	...
	assert.False(t, config.Planet.Admin.Enabled, "the example must not listen for operator tools")
	assert.Equal(t, "127.0.0.1:8081", config.Planet.Admin.BindAddress)
