...
- **A builder that returns a boolean is an adjective** — `empty`, `readable`, `negative`: `Account.Linked()`, `Providers.Off()`, `Client.Configured()`, `Account.linkedTo(provider)`. Not `IsLinked`, `HasProvider` or `holds`. A manipulator that reports whether it acted keeps its verb (`Session.ExtendIfDue` answers whether it extended), and the comma-ok second result of a builder (`CookieValue(…) (string, bool)`) is Go's idiom, not a name.
