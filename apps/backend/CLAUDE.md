...
- **Known risk: whole-script confusables.** The one-script rule stops a name that swaps one letter for a lookalike of another script, not a name written wholly in a script that looks like another: `Аԁа`, all Cyrillic, is not `Ada`, and both can be held. Closing it needs the UTS #39 skeletons (`confusables.txt`, vendored by a make target) folded into the unique key. The chat shows the tag beside every name, which is what tells two lookalikes apart today.
