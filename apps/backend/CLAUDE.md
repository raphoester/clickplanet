...
- `antiBot.shadowBan.banDurations`, `strikeMemory` — the ban for each offence (the last step repeats), and how long a served ban still counts. An offence is a ban that starts while none is running; a flag on a running ban only extends it
