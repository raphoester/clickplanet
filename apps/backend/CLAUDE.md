...
- **A manipulator returns no answer about what it did.** When a decision and an action go together, a builder hands back a small value holding the decision, and the manipulator takes that value: `extension := session.Extension(now, lifetime)`, then `if extension.Due() { session.Extend(extension) }`. Not `ExtendIfDue(...) bool`, which both acts and answers.
...
- **The rules are on `accounts.Session`**: `GuestSession` builds one with a full `guestTTL` (90 days) and `LinkedSession` with a full `linkedTTL` (30 days), `ExpiryError` is `ErrSessionExpired` past it, `Extension(now, lifetime)` is where using it would move the expiry (by the TTL of its kind) and whether that is `Due` (`extendEvery`, 24h, since the last extension), `Extend(extension)` moves it, and `Cookie` is its `Set-Cookie`. `Session.Linked` is read by the store (does the account have an identity), never written, so every session of an account extends as a linked one once any browser links it. `create_session_usecase` only orders them: attest, read the cookie, find the session, check it, extend and save it when due — or, when there is no cookie, no such session or an expired one, start and store a guest — then mint.
