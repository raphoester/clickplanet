...
The rest is still files on the `tile_state` volume, and moves to postgres next: bans (`antiBot.shadowBan.statePath`), antibot evidence (`antiBot.evidence.statePath`) and the chat log (`chat.storage.logPath`).
