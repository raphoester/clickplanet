...
**`cpdata/vpn_extra.txt` is ours**: VPN ranges X4BNet misses, kept by hand and folded into the `vpn` list. `make vpn-lists` does not touch it. It holds Mozilla's AS42037 (Mozilla VPN and Firefox's built-in VPN), which gives each browser session its own /64 — so one client walks the /32 and gets a fresh throttle bucket and ban scope each time.
